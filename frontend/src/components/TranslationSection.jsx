...
const TranslationSection = ({ transcriptData }) => {
  ...
  const copyToClipboard = (text) => {
    ...
    // Could add a small toast/state to show "Copied!" if needed
  };
    ...
    <div className="mt-8 border-t pt-6 border-gray-100 text-left animate-fade-in-up">
      ...
      {translations && Object.keys(translations).length > 0 && (
        <div className="mt-8 space-y-6">
          <h4 className="text-lg font-bold text-gray-900 border-b pb-2">Translated Versions</h4>
            ...
            <div key={langCode} className="bg-white border border-gray-200 rounded-lg shadow-sm p-5">
              <div className="flex justify-between items-center mb-3">
                <h5 className="font-semibold text-gray-900 text-lg">
                  {getLanguageName(langCode)} {text.startsWith('Error:') ? '✕' : '✓'}
                </h5>
              ...
              <div className={`p-4 rounded-lg bg-gray-50 max-h-60 overflow-y-auto whitespace-pre-wrap text-sm leading-relaxed ${text.startsWith('Error:') ? 'text-red-600' : 'text-gray-800'}`}>
                {text}
              </div>
            </div>
          ))}
